...
- `power.config.json` binds this local Code App to the Power Apps environment and lists the 18 custom Dataverse tables plus the built-in System User data source.
...
| [sfsu_white_label_and_reservable_hours_addendum.md](sfsu_white_label_and_reservable_hours_addendum.md) | Documents configurable App Name/logo upload, Resource Type/Resource reservable-hours schema and UI, calendar availability overlays, booking enforcement, and approval-flow handoff. |

## Dataverse Schema Documentation

Start with [sfsu_dataverse_build_sheet.md](sfsu_dataverse_build_sheet.md) for the original dependency-first build order, then read [sfsu_group_permissions_and_reservation_custom_fields_addendum.md](sfsu_group_permissions_and_reservation_custom_fields_addendum.md) for the reservation-answer extension, [sfsu_resource_attributes_and_custom_fields_addendum.md](sfsu_resource_attributes_and_custom_fields_addendum.md) for the current Resource Attribute/Custom Field implementation, [sfsu_reports_screen_and_saved_reports_addendum.md](sfsu_reports_screen_and_saved_reports_addendum.md) for the Saved Report extension, [sfsu_white_label_and_reservable_hours_addendum.md](sfsu_white_label_and_reservable_hours_addendum.md) for the Reservable Hour Window extension, and [sfsu_deployment_runbook.md](sfsu_deployment_runbook.md) for the Reservation Approval Request extension. The current solution has 18 custom tables:
...
15. Audit Log
16. Saved Report
17. Reservable Hour Window
18. Reservation Approval Request

Schema rules to preserve:

- Publisher prefix is `sfsures` and must remain stable across department instances.
- Reservation Series, Reservation Occurrence, Reservation Attribute Value, and Saved Report are User/team-owned; the other custom tables are Organization-owned.
...
- Resource Type includes local `sfsures_reservablehoursmode` choices for Any Time, Monday-Friday 8-5, and Custom.
- Resource includes local `sfsures_reservablehoursmode` choices for Inherit from Resource Type, Any Time, Monday-Friday 8-5, and Custom.
- Resource Type includes `Requires Approval`; Resource includes `Approval Mode` choices for inherit, no approval required, and approval required.
- Reservable Hour Window stores active weekly availability rows using day-of-week choice, start/end minute integers, display order, record status, and exactly one intended parent lookup to either Resource Type or Resource. Multiple windows per day are allowed.
- App Settings uses primary `sfsures_name` as the configurable App Name, includes Dataverse Image column `sfsures_applogo` for uploaded/cropped app logo data, and includes `Published App URL` for stable approval links in replicated environments. The old logo URL column is unused.
- Reservation Approval Request stores approval status, request type, requested start/end, requester/resource display snapshots, notification recipients, approval link, decision metadata, and links to the requested resource plus either an occurrence or series.
...
- All 18 custom Dataverse service/model files are generated, including Reservation Attribute Value, Saved Report, Reservable Hour Window, and Reservation Approval Request; the built-in System User source is also generated.
...
- `CalendarScreen` loads occurrences, blackout windows, active resource calendar colors, permitted Resource Types/Resources, reservable hour modes/windows, Attribute Definition labels, Resource Attribute values, and Custom Field answers with delegation-safe query shapes and renders them with FullCalendar, including configurable header branding, signed-in user profile/Gateway link, Help menu, Resource Type event filtering, the `View Resource` dropdown, resource-specific calendar filtering, `Show Info` handoff to the Resource Info modal, Week/Day non-reservable overlays, resource-colored reservation events with contrast-aware text, and a reservation detail dialog with owner profile, comments, Resource Attribute values, Custom Field answers, and edit/delete actions. It also handles `#/approval/{id}` deep links for App Admin approval review.
...
- `BookingModal` creates and updates single reservations and recurring reservation series against real Dataverse data, including Resource Type filtering above the Resource selector, Resource/Resource Type pre-selection from calendar context, disabled Resource/Resource Type selectors in edit modes, optional Comments, Admin owner selection, Text/Choice Custom Field answers, reservable-hours validation for single and recurring bookings, admin override confirmation outside regular hours, and a "custom availability" calendar handoff, then shows a centered confirmation state with `Edit Reservation` and focused `OK`. For approval-required resources, non-admin submissions create Pending reservation rows plus a populated Reservation Approval Request.
...
- Conflict detection against active or pending occurrences and blackout windows is implemented for single bookings and recurring occurrence generation; reservable-hours validation is app-enforced alongside those checks.
- Theme values load through `ThemeContext` from `sfsures_appsettings`, with configurable App Name and uploaded App Logo support plus portable bundled SFSU logo and fixed Source Sans 3 defaults as fallback.
- Reservation limits and `Published App URL` are loaded from `sfsures_appsettings`. Reservation limits have code hard caps: max 50 generated occurrences and max 18 weeks per reservation/series span. App Admins may configure more restrictive values only.
...
- App Settings screen is implemented with configurable App Name, `Published App URL`, SFSU preset themes, uploaded/cropped app logo image, and reservation limits. It intentionally excludes arbitrary colors, font selection, logo URL entry, border radius, and direct settings row-name editing beyond the App Name control.
...
- Verify the app-created approval flow end-to-end with a real non-admin Booker and App Admin, including email delivery, approval link routing, approve/deny status changes, and non-admin direct-link denial.
- Verify the calendar Resource Type filter, `View Resource` dropdown resource filtering, `Show Info` modal handoff, full-size Resource photo preview, `Reserve This Resource` handoff, and reservable-hours overlays in the published Power Apps runtime.
- Verify Resource Attribute values and Custom Field answers with a non-admin Booker identity, including role privileges.
- Test Viewer-only access in the published app.
...
- Non-admin Booker test passed on 2026-07-31 for login, own single/recurring reservation create/delete, and peer reservation delete blocking.
